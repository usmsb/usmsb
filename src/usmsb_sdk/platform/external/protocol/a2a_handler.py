...
This module provides the handler for Agent-to-Agent (A2A) protocol,
enabling direct communication between AI agents.

A2A Protocol Features:
- Direct agent-to-agent messaging
- Skill discovery and capability exchange
...
warnings.warn(
    "platform.external.protocol.a2a_handler is deprecated. "
...
class A2AProtocolHandler(BaseProtocolHandler):
    ...
    This handler implements direct agent-to-agent communication following
    ...
    def __init__(
        ...
        logger.info(f"A2AProtocolHandler initialized with agent_id={self._agent_id}")
    ...
    async def _do_call_skill(
        ...
        # Create A2A envelope using centralized A2AEnvelope
    ...
    async def _handle_pong(self, message: ProtocolMessage) -> ProtocolResponse | None:
        """Handle pong response."""
        logger.debug("A2A received pong")
        return None

    async def _handle_discover_response(
        self,
    ...
    async def _handle_skill_response(
        self,
        message: ProtocolMessage,
    ) -> ProtocolResponse | None:
    ...
    async def _handle_error(self, message: ProtocolMessage) -> ProtocolResponse | None:
