...
export default function RequestsPage() {
  ...
  let requests: InferenceRequest[] = mockRequests
  if (data?.requests && data.requests.length > 0) {
    requests = data.requests as InferenceRequest[]
  } else if (query) {
    requests = mockRequests.filter(r =>
      r.request_id.toLowerCase().includes(query.toLowerCase()) ||
      r.model_name.toLowerCase().includes(query.toLowerCase())
    )
  }
  ...
  const columns = [
    ...
    },
    {
      key: 'node_id',
      header: 'Node',
      render: (r: InferenceRequest) => <span className="font-mono text-xs">{r.node_id}</span>,
    },
    {
      key: 'status',
      header: 'Status',
      render: (r: InferenceRequest) => <Badge status={r.status} dot />,
    },
    {
      key: 'tokens',
      header: 'Tokens',
      render: (r: InferenceRequest) => <span className="font-mono text-xs">{r.input_tokens + r.output_tokens}</span>,
    },
    {
      key: 'latency',
      header: 'Latency',
      render: (r: InferenceRequest) => <span className="font-mono text-xs text-text-secondary">{r.latency_ms > 0 ? formatLatency(r.latency_ms) : '-'}</span>,
    },
    {
      key: 'cost',
      header: 'Cost',
      render: (r: InferenceRequest) => <span className="font-mono text-xs text-neon-green">{r.cost_vibe > 0 ? formatVibe(r.cost_vibe) : '-'}</span>,
    },
    {
      key: 'time',
      header: 'Time',
      render: (r: InferenceRequest) => <span className="text-xs text-text-secondary">{formatTime(r.created_at)}</span>,
    },
  ]
